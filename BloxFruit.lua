	...
	local Main = library:Window("xUraquiam","Blox Fruits")
	...
	AutoFarmTab:Toggle("Fast Attack", true,function(vu)
		_G.FastAttack = vu
	end)
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    AutoFarmTab:Toggle("Auto Setspawn",false,function()
        local args = {
            [1] = "SetSpawnPoint"
        }
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
...
AutoFarmTab:Toggle("White Screen",false,function(vu)
    _G.WhiteScreen = vu
...
AutoFarmTab:Line()
	...
	local SelectedWeapon = AutoFarmTab:Dropdown("Select Weapon",Weapon,0,function(a)
		SelectToolWeapon = a 
	end)
	AutoFarmTab:Button("Refrash Weapon", function()
		Weapon = {}
		for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do  
			if v:IsA("Tool") then
				table.insert(Weapon ,v.Name)
	...
	AutoFarmTab:Toggle("Auto FullySuperhuman" , getgenv().Setting["Auto Superhuman [Full]"],function(vu)
		SuperhumanFull = vu
	end)
	-- Auto Death Step
	...
	SelectBoss = ""
	local BossName = AutoFarmTab:Dropdown("Select Boss",Boss,0,function(Value)
		SelectBoss = Value
		Don = false
	end)
	AutoFarmTab:Button("Refresh Boss",function()
		local Boss = {}
		for i, v in pairs(game.ReplicatedStorage:GetChildren()) do
			if string.find(v.Name, "Boss") then
				if v.Name == "Ice Admiral [Lv. 700] [Boss]" then
	...
	AutoFarmTab:Toggle("Auto Farm Boss",false,function(Value)
		local args = {
			[1] = "AbandonQuest"
		}
		game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
	...
	local BossNameHop = AutoFarmTab:Dropdown("Select Boss To Farm Hop",Boss,0,function(Value)
	...
	AutoFarmTab:Toggle("Auto Farm Select Boss Hop",getgenv().Setting["Auto Farm Select Boss Hop"],function(Value)
	...
	AutoFarmTab:Toggle("Auto Farm All Boss",false,function(Value)
		KillAllBoss = Value
		MsBoss = ""
		KillBossuseGet = false
	end)
	...
	AutoFarmTab:Line()
	-- Auto Farm Near Mods
	AutoFarmTab:Toggle("Auto Farm Mob Aura",false,function(v)
		AutoMobAura = v 
	...
	AutoFarmTab:Slider("Distance Mob",1,400,100,DistanceMobAura,function(A)
		DistanceMobAura = A
	end)
...
--------------------------------------------------------------------------------------------------------------------
	...
	----------------------------------------------------------------------------------------------------------------------------
	local PlayerEspTab = Main:Tab("Combat")
	...
	MiscTab:Button("Unlock Fps",function()
		setfpscap(1000)
	end)
	
	MiscTab:Button("Pro Graphic",function()
